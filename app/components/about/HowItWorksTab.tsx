...
export function HowItWorksTab() {
  ...
  const SECTIONS: { key: string; label: string; content: React.ReactNode }[] = [
    ...
    { key: "database", label: "Company Database", content: (
      <>
        <p style={ps}>Your saved companies live here. Use the filters at the top (geography, company category, source, highest product price, ICP fit, priority tier), then <strong>Find Companies</strong> to apply them, or <strong>Show All Companies</strong>. Geography, company category and priority tier let you pick several values at once. Click a column heading to sort the table by it; by default the newest-added companies show first. Click a row to expand its description. Each row shows the <strong>date added</strong> and an editable <strong>Status</strong> (Not contacted, Contacted, In dialogue, Not interested, Not relevant) for tracking outreach; it saves the moment you change it.</p>
        <ul style={uls}>
          <li style={lis}><strong>+ Add Company:</strong> manually add a company (name required, plus website, geography, company category, price, tier, ICP fit, and notes) without running a search. Saved straight to the database.</li>
          ...
          <li style={lis}><strong>Edit list:</strong> turns on edit mode. Each row gets a pencil (edit its fields) and an ✕ (choose <em>Remove from this view only</em>, which is hidden and restorable, or <em>Delete from the company database</em>).</li>
    ...
    { key: "finding", label: "Finding new companies", content: (
      <>
        <p style={ps}>On <strong>Find New Companies</strong> there are two ways to search, side by side:</p>
        <ul style={uls}>
          <li style={lis}><strong>Search for new companies</strong> (left): pick up to 3 search terms and up to 4 sources (or leave them unticked to use the defaults), then run it. The app finds new companies, then researches and scores the newest 5.</li>
          <li style={lis}><strong>Research the waiting list</strong> (right): research and score companies that were already found in earlier searches but not yet researched. It doesn&apos;t look for anything new. See <em>The waiting list</em>.</li>
        </ul>
        <p style={ps}>You use one or the other: ticking companies in the waiting list locks the new-search side, and picking terms or sources locks the waiting-list side. A search takes about <strong>15 minutes</strong> and stops automatically after 30. While it runs you&apos;ll see &quot;Step X of 3&quot;, a timer, and an expandable <strong>Search Log</strong> that mirrors exactly what the app is doing.</p>
        <p style={{ fontWeight: 700, color: "var(--navy)", margin: "16px 0 6px" }}>What happens behind the scenes: three steps</p>
        <ul style={uls}>
          <li style={lis}><strong>1. Discovery:</strong> the AI runs web searches (and reads any single-page or YouTube sources you picked) using your terms, and extracts supplement company or brand names it hasn&apos;t seen before. Anything already in your database, rejected, or already waiting is filtered out. New names go into the waiting list. (This step only runs for a new search, not when you research the waiting list.)</li>
          <li style={lis}><strong>2. Research:</strong> for up to 5 companies at a time, the AI does its own web searches to gather details: their website, what they sell, whether they do omega-3 or krill, how they describe themselves, price level, which European markets, and sales channels. Each company is saved the moment its research finishes, so nothing is lost partway.</li>
          <li style={lis}><strong>3. Scoring (ICP):</strong> the AI reads everything gathered and scores each company against the Lysoveta ICP, giving a fit score, a priority tier (Early Mover or Follower), and a short reason. Only companies that pass are shown for you to review and save.</li>
        </ul>
        <p style={ps}>When it finishes, tick the companies you want and click <strong>Add to Database</strong>. That opens a <strong>Fill in Details</strong> step where you can review and adjust each company&apos;s fields before saving. If one turns out not to fit on a closer look, its <strong>Remove ✕</strong> drops it. Then save the rest. (After a search, your term and source ticks reset for a clean next run.)</p>
        <p style={muted}>The first search after a quiet period can take about 30 seconds to start, because the server &quot;wakes up&quot; after being idle. That&apos;s normal.</p>
      </>
    ) },
    { key: "config", label: "Search terms & sources", content: (
      <>
        ...
        <p style={ps}>A <strong>single page</strong> is read once, so after a search it moves into a <strong>&quot;Completed single pages&quot;</strong> list and out of the selectable list, so you can&apos;t waste a search on it. If you think a page has since been updated, open that list and click <strong>Add back to source list</strong>.</p>
        <p style={ps}>You can pick up to 3 terms and 4 sources per search. The limit isn&apos;t arbitrary: a search runs <em>terms × website-sources</em> web searches with a budget of 12, and 3 × 4 = 12 fills it exactly, so picking more can&apos;t all run.</p>
        <p style={{ fontWeight: 700, color: "var(--navy)", margin: "16px 0 6px" }}>How well is each source doing?</p>
        <p style={ps}>Under every source you&apos;ll see a small line, for example <strong>&quot;used 5 · found 12 · saved 2&quot;</strong>, so you can tell which sources actually pull their weight:</p>
        <ul style={uls}>
          <li style={lis}><strong>used:</strong> how many searches this source has taken part in.</li>
          <li style={lis}><strong>found:</strong> how many new companies it has added to the waiting list over time.</li>
    ...
    { key: "queue", label: "The waiting list", content: (
      <>
        <p style={ps}>Companies are researched in small batches of <strong>5 at a time</strong>, because researching each one is the slow, costly step. When a new search finds more than 5 companies, the extras wait in a list until you research them.</p>
        <p style={ps}>The <strong>Waiting list</strong> box (on the Find New Companies tab) shows the companies waiting. To research them, tick up to 5 and click <strong>Research the waiting list</strong>, or leave them unticked to take the 5 that have waited longest. This is separate from the search configuration: it researches and scores companies that were already found, without looking for new ones.</p>
      </>
    ) },
    { key: "scoring", label: "How scoring works (ICP)", content: (
      <>
        <p style={ps}>After a company is researched, the AI scores it against the <strong>Lysoveta ICP Criteria</strong> (see that tab). It assigns an ICP fit score and a priority tier (Early Mover or Follower).</p>
        ...
        <p style={ps}>Next to the ICP is a <strong>Company categories</strong> card, the one place to manage the list of categories a company can be tagged with (used in the Company Database and suggested by the AI after a search). Click <strong>✎ Manage</strong> to add, rename, or remove them. The priority tiers (Early Mover, Follower) are fixed and not edited here.</p>
      </>
    ) },
    { key: "exceptions", label: "When something goes wrong", content: (
      <>
        ...
        <ul style={uls}>
          <li style={lis}><strong>&quot;No new companies found&quot;:</strong> everything found was already in your database, rejected, or waiting. The sources may not have published anything new, or the terms keep hitting the same companies. Try again later, or adjust and add sources and terms.</li>
    ...
    { key: "tips", label: "Tips & good to know", content: (
      <>
        <ul style={uls}>
          ...
          <li style={lis}>The log-in is a <strong>simple pilot gate</strong>, not real security (see <em>Signing in</em>), so please don&apos;t share the link more widely than intended.</li>
